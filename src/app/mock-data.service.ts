...
export class MockDataService {
  ...
  getCharacters(searchTerm?: string): Observable<string> {
    this.charactersLoader$.next(true)
    const queryParams: string = searchTerm ? `?search=${searchTerm}` : ""
    return this.httpClient
      .get<Record<string, string>(`https://swapi.dev/api/people/${queryParams}`)
      .pipe(finalize(() => this.charactersLoader$.next(false)))
      .pipe(map((response) => response.results))
  }

  getPlatents(searchTerm?: string): Observable<string> {
    this.planetsLoader$.next(true)
    const queryParams: string = searchTerm ? `?search=${searchTerm}` : ""
    return this.httpClient
      .get<Record<string, string>>(`https://swapi.dev/api/planets/${queryParams}`)
